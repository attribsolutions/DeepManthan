...
BEGIN
    ...
    declare PartyType int(10);
        ...
        if PartyID > 0 then
			SELECT id, MRP into IDD,MRPP FROM M_MRPMaster where Item_id=ItemID
            and EffectiveDate<=EffectiveDate1 and Party_id=PartyID and PartyType_id is null order by EffectiveDate desc, id desc limit 1;
		end if;    
        if MRPP is null  then
            
			if PartyType > 0 then
				SELECT id, MRP into IDD,MRPP FROM M_MRPMaster where Item_id=ItemID
				and EffectiveDate<=EffectiveDate1 and Party_id is null and PartyType_id = PartyType order by EffectiveDate desc, id desc limit 1;	
			end if;
            if MRPP is null  then
				SELECT id, MRP into IDD,MRPP FROM M_MRPMaster where Item_id=ItemID
				and EffectiveDate<=EffectiveDate1 and Party_id is null and PartyType_id is null order by EffectiveDate desc, id desc limit 1;
			END IF;

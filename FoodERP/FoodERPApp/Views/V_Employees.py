...
JOIN M_Districts ON M_Districts.id=M_Employees.District_id where M_Employees.id= %s''', [id])
                if not query:
                    ...
                    Employeeparties = MC_EmployeeParties.objects.raw('''SELECT Party_id as id, m_parties.Name as lable FROM mc_employeeparties join m_parties on m_parties.id = mc_employeeparties.Party_id where Employee_id=%s''', ([id]))

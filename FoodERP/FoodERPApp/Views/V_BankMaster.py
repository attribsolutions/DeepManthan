    ...
    def post(self, request, id=0):
        try:
            with transaction.atomic():
                ...
                query1 = MC_PartyBanks.objects.raw('''SELECT MC_PartyBanks.id, M_Bank.id AS Bank_id, MC_PartyBanks.IFSC, MC_PartyBanks.BranchName, MC_PartyBanks.AccountNo, MC_PartyBanks.IsSelfDepositoryBank, MC_PartyBanks.Company_id, MC_PartyBanks.Party_id, MC_PartyBanks.IsDefault FROM M_Bank LEFT JOIN MC_PartyBanks ON MC_PartyBanks.Bank_id = M_Bank.id AND MC_PartyBanks.Party_id=%s WHERE M_Bank.Company_id=%s''', ([Party], [Company]))

                if query1:
                    bank_serializer = PartyBanksSerializer(
                        query1, many=True).data
                    ReceiptListData = list()
                    for a in bank_serializer:
                        ReceiptListData.append({
                            "id": a['id'],
                            "Bank": a['ReceiptDate'],
                            "IFSC": a['FullReceiptNumber'],
                             "BranchName": a['BranchName'],
                            "AccountNo": a['AccountNo'],
                            "IsSelfDepositoryBank": a['IsSelfDepositoryBank'], 
                            "IsDefault": a['IsDefault'],
                            "CustomerBank": a['Party'],
                            "Company": a['Company'],
                            "BankName": a['BankName'],
    ...
    def post(self, request, id=0):
        try:
            with transaction.atomic():
                ...
                Party = Bank_data['PartyID']
                Company = Bank_data['CompanyID']
                query = MC_PartyBanks.objects.filter(
    ...
    def post(self, request, id=0):
        try:
            with transaction.atomic():
                ...
                PartyBanks_serializer = PartyBanksSerializerSecond(
                    data=PartyBanks_data, many=True)

...
O_LiveBatches.BatchCode,O_LiveBatches.SystemBatchCode,RateCalculationFunction1(0, M_Items.id, %s, %s, 0, 0, O_LiveBatches.MRPValue, 0)Rate,GSTHsnCodeMaster(M_Items.id,%s,2)GSTPercentage
FROM M_Items 
...
where MC_PartyItems.Party_id=%s and O_BatchWiseLiveStock.BaseUnitQuantity >0 and O_BatchWiseLiveStock.IsDamagePieces=%s  order by M_Group.id, MC_SubGroup.id ,M_Items.id ''',([Party],[Unit],today,[Party],[Party],[IsDamagePieces]))
                ...
                if not Itemquery:
                    ...
                    for row in Itemquery:
                        ...
                        Valuee=float(ActualQty) * float(row.Rate)
                        withGSTRate= float(Valuee) +(float(Valuee) *(float(row.GSTPercentage)/100))

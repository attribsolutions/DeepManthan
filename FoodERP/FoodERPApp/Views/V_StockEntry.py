...
O_LiveBatches.BatchCode,O_LiveBatches.SystemBatchCode,RateCalculationFunction1(0, M_Items.id, %s, %s, 0, 0, 0, 0)Rate,(RateCalculationFunction1(0, M_Items.id, %s, %s, 0, 0, 0, 0)* O_BatchWiseLiveStock.BaseUnitQuantity)Valuee
FROM M_Items 
...
where MC_PartyItems.Party_id=%s and O_BatchWiseLiveStock.BaseUnitQuantity >0 and O_BatchWiseLiveStock.IsDamagePieces=%s and M_Items.id=6 order by M_Group.id, MC_SubGroup.id ,M_Items.id ''',([Party],[Unit],[Party],[Unit],[Party],[Party],[IsDamagePieces]))

                ...
                try:
                    if(PriceListID):
                        ...
                        if len(PartyTypeID) > 0:
                           ...
                           PriceListID = [str(item.id) for item in pricelistPartytype]
                    ...
                    for row in ItemQuery:
                        ...
                        if IsSCM == '0':
                            ...
                            if PriceListID == 0:
                                ...
                                # CustomPrint("Shruti")
                                pricelistquery=M_PriceList.objects.raw('''SELECT id,Name,ShortName,CalculationPath FROM M_PriceList where id  in  %s order by Sequence''',[PriceListID])
        ...
        try:
            with transaction.atomic():
                ...
                SPOSInvoicequery ='''Select X.id, M_Parties.Name FranchiseName, M_Parties.SAPPartyCode SAPCode, M_Parties.Name SAPName, X.InvoiceDate SaleDate, 
                        X.ClientID, M_Items.CItemID , X.FullInvoiceNumber BillNumber, M_Items.Name ItemName, Y.Quantity, M_Units.Name UnitName, Y.MRPValue Rate, Y.Amount, M_Items.IsCBMItem, X.MobileNo, M_Items.SAPItemCode MaterialSAPCode

    ...
    def get(self, request):
        try:
            with transaction.atomic():
                query = M_PriceList.objects.raw('''SELECT m_pricelist.id,m_pricelist.Name,m_partytype.Name PartyTypeName FROM m_pricelist
JOIN m_partytype ON m_partytype.id=m_pricelist.PLPartyType_id''')
                ...
                if not query:
                    ...
                    PriceList_Serializer = PriceListSerializer2(query, many=True).data
    ...
    def post(self, request):
        try:
            with transaction.atomic():
                ...
                PriceListdata_Serializer = PriceListSerializer(data=PriceListdata)
...
WHERE m_pricelist.id = %s''',[id])
                if not query:
                    ...
                    PriceListdata_Serializer = PriceListSerializer2(query, many=True).data

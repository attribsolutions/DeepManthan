    ...
    def get(self, request ):
        try:
            with transaction.atomic():
                Bomsdata = M_BillOfMaterial.objects.all()
                if Bomsdata.exists():
                    Boms_Serializer = H_ModulesSerializer(
                    Bomsdata, many=True)
                    return JsonResponse({'StatusCode': 200, 'Status': True,'Message': '','Data': Boms_Serializer.data })
                return JsonResponse({'StatusCode': 204, 'Status': True,'Message':  'BOM  Not available', 'Data': []})    
        except Exception :
    ...
    def post(self, request):
        try:
            with transaction.atomic():
                ...
                Boms_Serializer = H_ModulesSerializer(data=Bomsdata)
    ...
    def get(self, request, id=0):
        try:
            with transaction.atomic():
                ...
                Boms_Serializer = H_ModulesSerializer(Bomsdata)
                return JsonResponse({'StatusCode': 200, 'Status': True,'Message': '','Data': Boms_Serializer.data})
    ...
    def put(self, request, id=0):
        try:
            with transaction.atomic():
                ...
                Boms_Serializer = H_ModulesSerializer(BomsdataByID, data=Bomsdata)
                if Boms_Serializer.is_valid():
                    Boms_Serializer.save()

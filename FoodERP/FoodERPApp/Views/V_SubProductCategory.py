...
JOIN m_productcategory ON m_productcategory.id=m_productsubcategory.ProductCategory_id''')
                if not query:
                    return JsonResponse({'StatusCode': 204, 'Status': True,'Message': 'Product SubCategory Not available', 'Data': []})
                else:    
        ...
        try:
            with transaction.atomic():
                ...
                if SubProductCategory_Serializer.is_valid():
                    ...
                    return JsonResponse({'StatusCode': 200, 'Status': True, 'Message': 'Product SubCategory Save Successfully', 'Data':[]})
                else:
        ...
        try:
            with transaction.atomic():
                ...
                if SubProductCategorydata_Serializer.is_valid():
                    ...
                    return JsonResponse({'StatusCode': 200, 'Status': True, 'Message': 'Product SubCategory Updated Successfully', 'Data':[]})
                else:
                    transaction.set_rollback(True)
    ...
    def delete(self, request, id=0):
        try:
            with transaction.atomic():
                ...
                return JsonResponse({'StatusCode': 200, 'Status': True, 'Message': 'Product SubCategory Deleted Successfully', 'Data':[]})
        except M_PartyType.DoesNotExist:
            return JsonResponse({'StatusCode': 204, 'Status': True, 'Message':'Product SubCategory Not available', 'Data': []})
        except IntegrityError:   
            return JsonResponse({'StatusCode': 204, 'Status': True, 'Message':'Product SubCategory used in another table', 'Data': []})   

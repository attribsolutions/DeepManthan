    ...
    def delete(self, request, id=0):
        try:
            with transaction.atomic():
                ...
                Receiptdata.delete()
                log_entry = create_transaction_log(request, {'ReceiptID':id}, 0, 0,  "Receipt Deleted Successfully",80,0)
                return JsonResponse({'StatusCode': 200, 'Status': True, 'Message': 'Receipt Deleted Successfully', 'Data': []})
        except IntegrityError:
            log_entry = create_transaction_log(request, {'ReceiptID':id}, 0, 0,  "Used in another table",8,0)
            return JsonResponse({'StatusCode': 204, 'Status': True, 'Message': 'Receipt used in another table', 'Data': []})
        except Exception as e:
            log_entry = create_transaction_log(request, {'ReceiptID':id}, 0, 0,  Exception(e),33,0)
            return JsonResponse({'StatusCode': 400, 'Status': True, 'Message':  Exception(e), 'Data': []})

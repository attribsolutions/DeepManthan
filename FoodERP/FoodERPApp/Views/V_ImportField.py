    ...
    def post(self, request):
        try:
            with transaction.atomic():
                ...
                if Party =="":
                    query = M_ImportFields.objects.raw('''SELECT M_ImportFields.id,M_ImportFields.Sequence, M_ImportFields.FieldName, M_ImportFields.IsCompulsory,M_ImportFields.ControlType_id, M_ImportFields.FieldValidation_id,MC_PartyImportFields.Value,MC_PartyImportFields.Party_id,M_ControlTypeMaster.Name ControlTypeName,M_FieldValidations.Name FieldValidationName,M_FieldValidations.RegularExpression FROM M_ImportFields Left JOIN MC_PartyImportFields ON M_ImportFields.id = MC_PartyImportFields.ImportField_id JOIN M_ControlTypeMaster ON M_ControlTypeMaster.id = M_ImportFields.ControlType_id JOIN M_FieldValidations ON M_FieldValidations.id = M_ImportFields.FieldValidation_id''') 
                else:    

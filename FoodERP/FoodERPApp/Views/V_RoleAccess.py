...
class RoleAccessViewNewUpdated(RetrieveAPIView):
    ...
    def get(self, request,Role=0,Division=0 ,Company=0):
        if int(Division) > 0:
            roleaccessquery = M_RoleAccess.objects.raw('''SELECT m_roleaccess.id id, h_modules.id moduleid, h_modules.Name ModuleName,m_pages.id pageid,m_pages.RelatedPageID, m_pages.name PageName  FROM m_roleaccess JOIN m_pages ON m_pages.id=m_roleaccess.Pages_id JOIN h_modules ON h_modules.id=m_roleaccess.Modules_id Join m_pagetype on m_pagetype.id= m_pages.PageType  WHERE m_pagetype.IsAvailableForAccess=1 AND Role_id=%s AND Division_id=%s AND Company_id=%s Order By h_modules.Name , m_pages.name ''',([Role],[Division],[Company]))
        else:
            roleaccessquery = M_RoleAccess.objects.raw('''SELECT m_roleaccess.id id, h_modules.id moduleid, h_modules.Name ModuleName,m_pages.id pageid,m_pages.RelatedPageID, m_pages.name PageName  FROM m_roleaccess JOIN m_pages ON m_pages.id=m_roleaccess.Pages_id JOIN h_modules ON h_modules.id=m_roleaccess.Modules_id Join m_pagetype on m_pagetype.id= m_pages.PageType  WHERE m_pagetype.IsAvailableForAccess=1 AND Role_id=%s AND Division_id is null AND Company_id=%s Order By h_modules.Name , m_pages.name ''',([Role],[Company]))            

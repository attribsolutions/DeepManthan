...
class RoleAccessViewList(RetrieveAPIView):
    ...
    left join C_Companies  ON C_Companies.id=M_RoleAccess.Company_id where M_RoleAccess.CreatedBy=%s  group by Role_id,Division_id,Company_id''',[UserID]) 
                ...
                if not query:
                    log_entry = create_transaction_logNew(request, Logindata,0, 'RoleAccess List Not Found',129,0)
                    ...
                    log_entry = create_transaction_logNew(request, Logindata,0,'Company:'+str(CompanyID),129,0)
    ...
    def get(self, request,Role=0,Division=0 ,Company=0):
        ...
        for a in RoleAccessdata:
            ...
            Moduledata.append({
                ...
                "RoleAccess_E-WayBill Upload": PageAccessSerializer[13]['id'],
                "RoleAccess_E-WayBill cancel": PageAccessSerializer[14]['id'],
                "RoleAccess_E-WayBill Print": PageAccessSerializer[15]['id'],
                "RoleAccess_E-Invoice Upload": PageAccessSerializer[16]['id'],
                "RoleAccess_E-Invoice cancel": PageAccessSerializer[17]['id'],
                "RoleAccess_E-Invoice Print": PageAccessSerializer[18]['id'],
        ...
        log_entry = create_transaction_logNew(request, 0,0,"RoleAccessNewUpdated",130,0)
        return Response(response)
    ...
    def get(self, request, moduleid=0,Division=0):
        try:
            with transaction.atomic():
                ...
                if not query:
                    log_entry = create_transaction_logNew(request,0,0,"RoleAccessGetPagesOnModule Not Found",133,0)
                    return JsonResponse({'StatusCode': 200, 'Status': True, 'Message': 'Records Not Found', 'Data': []})
                else:
